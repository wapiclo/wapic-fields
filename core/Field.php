...
class Field {
    ...
    private function control_select2($required) {
        ...
        $placeholder = !empty($this->attributes['placeholder']) ? $this->attributes['placeholder'] : __('Select an option', 'wapic-fields-text');
    ...
    private function control_image($required) {
        ...
        if ($img_url) {
            ...
            echo '<a href="#" class="wcf-field-remove-image" title="' . esc_attr__('Remove image', 'wapic-fields-text') . '">×</a>';
            echo '</span>';
        }
        echo '</div>';
        printf(
        ...
        if (!empty($ids)) {
            foreach ($ids as $img_id) {
                ...
                if ($img_url) {
                    ...
                    echo '<img src="' . esc_url($img_url) . '" alt="' . esc_attr__('Gallery image', 'wapic-fields-text') . '">';
                    echo '<a href="#" class="wcf-field-remove-gallery-thumb" title="' . esc_attr__('Remove image', 'wapic-fields-text') . '">×</a>';
                    echo '</span>';
                }
            }
        }
    ...
    private function control_media_upload($required) {
        ...
        if ($img_url) {
            ...
            echo '<a href="#" class="wcf-media-remove" title="' . esc_attr__('Remove', 'wapic-fields-text') . '">×</a>';
            echo '</div>';
        }
        echo '</div>';
        ...
        // URL input
        printf(
            '<input type="text" class="wcf-field__input wcf-media-url" id="%s_url" value="%s" placeholder="' . esc_attr__('Media URL', 'wapic-fields-text') . '" readonly />',
            esc_attr($this->id),
            esc_url($img_url)
        );
        ...
        // Upload button
        echo '<button type="button" class="button wcf-media-upload-button" data-target="' . esc_attr($this->id) . '">' . esc_html__('Select/Upload Media', 'wapic-fields-text') . '</button>';
    ...
    private function control_file($required) {
        ...
        printf(
            '<input type="text" id="%1$s" name="%2$s" value="%3$s" class="wcf-field-file-url wcf-field__input" placeholder="' . esc_attr__('File URL', 'wapic-fields-text') . '" data-validate="url" %4$s/>',
            esc_attr($this->id),
            esc_attr($this->name),
            esc_url($value),
            $required ? 'required' : ''
        ...
        echo '<button type="button" class="button wcf-file-upload-button" data-target="' . esc_attr($this->id) . '">' . esc_html__('Select File', 'wapic-fields-text') . '</button>';
        echo '</div>';
    }
    ...
    public function validate_value($type, $value) {
        ...
        switch ($type) {
            case 'email':
                if (!is_email($value)) {
                    $error = __('Please enter a valid email address', 'wapic-fields-text');
                }
                break;
            ...
            case 'price':
                if (!is_numeric($value)) {
                    $error = __('Please enter a valid number', 'wapic-fields-text');
                }
                break;

            case 'phone':
                if (!preg_match('/^[0-9+\-\s()]+$/', $value)) {
                    $error = __('Please enter a valid phone number', 'wapic-fields-text');
                }
                break;

            case 'url':
                if (!filter_var($value, FILTER_VALIDATE_URL)) {
                    $error = __('Please enter a valid URL', 'wapic-fields-text');
                }
                break;
        }
    ...
    // Panel render 
    public function start_controls_panel($args = []) {

        $args = wp_parse_args($args, [
            'title' => __('Panel', 'wapic-fields-text'),
            'id'    => 'custom-panel',
            'type'  => 'setting',
        ]);
    ...
    public function end_controls_panel($args = []) {
