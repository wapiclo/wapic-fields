...
class File extends Field {
    ...
    protected function render_input(string $required_attr): void {
        ...
        printf(
            '<input type="text" id="%1$s" name="%2$s" value="%3$s" class="wcf-field-file-url wcf-field__input" placeholder="%4$s" data-validate="url" %5$s/>',
            esc_attr($this->id),
            esc_attr($this->name),
            esc_url((string) $this->value),
            esc_attr__('File URL', 'wapic-fields'),
